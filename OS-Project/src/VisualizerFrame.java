...
public class VisualizerFrame extends JFrame {
	...
	private final int MAX_SIZE = 500;   //to be changed after
...
//		});
		...
		size.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				validate();
				SortApp.sortDataCount = size.getValue();
			}
		});
	...
	public void preDrawArray(ArrayList<Integer> squares){
		squarePanels = new JPanel[SortApp.sortDataCount];
		arrayWrapper.removeAll();
		// 90% of the windows height, divided by the size of the sorted array.
		sizeModifier =  (int) ((getHeight()*0.9)/(squarePanels.length));
		for(int i = 0; i<SortApp.sortDataCount; i++){
			squarePanels[i] = new JPanel();
			squarePanels[i].setPreferredSize(new Dimension(SortApp.blockWidth, squares.get(i)*sizeModifier));
	...
	public void reDrawArray(ArrayList<Integer> x){
		reDrawArray(x, -1);
	}

	public void reDrawArray(ArrayList<Integer> x, int y){
		reDrawArray(x, y, -1);
	}

	public void reDrawArray(ArrayList<Integer> x, int y, int z){
		reDrawArray(x, y, z, -1);
	}
...
//		System.out.println("frame");
		arrayWrapper.removeAll();
		System.out.println(squarePanels.length);
